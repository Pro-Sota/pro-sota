...
import MemberSubmissionsPage from "./team_view";
...
function isOverdue(s: Submission) {
  if (s.status !== "Em Revisão" && s.status !== "Mudanças Solicitadas")
    return false;
...
function ActionButton({
  ...
  const tones = {
    green:
      "text-green-700 hover:bg-green-50 focus-visible:ring-green-500 disabled:text-green-400",
...
function ToastNotification({
  ...
  return (
    <div
      className={`${colors[toast.type]} text-white px-5 py-3 rounded-xl shadow-2xl text-sm flex items-center justify-between gap-3`}
    >
...
export default function ApprovalsAndReviewsPage() {
  ...
  const skipInitialSubmissionsSave = useRef(true);
  const skipInitialUndoSave = useRef(true);
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (saved) {
        timers.push(
          window.setTimeout(() => setSubmissions(JSON.parse(saved)), 0),
        );
      }
      const savedUndo = localStorage.getItem(UNDO_STACK_KEY);
      if (savedUndo) {
        timers.push(
          window.setTimeout(() => setUndoStack(JSON.parse(savedUndo)), 0),
        );
      }
    } catch (e) {
  ...
  useEffect(() => {
    if (skipInitialSubmissionsSave.current) {
      skipInitialSubmissionsSave.current = false;
      return;
    }
  ...
  // Save undo stack
  useEffect(() => {
    if (skipInitialUndoSave.current) {
      skipInitialUndoSave.current = false;
      return;
    }
    localStorage.setItem(
      UNDO_STACK_KEY,
      JSON.stringify(undoStack.slice(0, 10)),
    ); // Keep last 10
  }, [undoStack]);
  ...
  const counts = useMemo(
    () => ({
      ...
      changesRequested: submissions.filter(
        (s) => s.status === "Mudanças Solicitadas",
      ).length,
  ...
  const handleUndo = () => {
    ...
    if (last.action === "delete") {
      ...
      setSubmissions((prev) =>
        prev.map((s) =>
          s.id === last.submission.id
            ? { ...s, status: last.previousStatus! }
            : s,
        ),
      );
      showToast(
        `Desfeito: "${last.submission.title}" → ${last.previousStatus}`,
        "info",
      );
    }
    ...
    >
        ...
        <div className="p-4 border-b border-gray-200 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <select
              value={typeFilter}
              onChange={(e) =>
                setTypeFilter(e.target.value as SubmissionType | "all")
              }
      ...
      {detailItem && (
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-label="Detalhes da submissão"
        >
          <div
      ...
      {showNewModal && (
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-label={editItem ? "Editar submissão" : "Nova submissão"}
        >
          <div
            className="bg-white rounded-xl shadow-xl max-w-md w-full p-6 space-y-4"
            onClick={(e) => e.stopPropagation()}
          >
            ...
            <div className="flex justify-end gap-2 pt-2">
              ...
              <button
                ...
                disabled={
                  !newTitle.trim() || !newSubmitter.trim() || !newDueDate
                }
